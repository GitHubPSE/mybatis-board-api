...
Spring Boot + MyBatis + PostgreSQL 기반의 게시판 CRUD 웹 애플리케이션입니다. JWT 기반 회원가입/로그인과 작성자 본인만 수정/삭제할 수 있는 권한 체크를 포함합니다.
...
| Method | URL | 설명 | 인증 필요 |
|--------|-----|------|----------|
| GET | `/api/board/list?page={page}&pageSize={size}` | 게시글 목록 조회 (페이징) | X |
